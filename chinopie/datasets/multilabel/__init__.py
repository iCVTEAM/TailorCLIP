...
class MultiLabelLocalDataset(MultiLabelDataset):
    ...
    def filter_by_labels(self, label_ids:List[int], strict:bool=False):
        annotations=self.get_all_labels()
        res_ids=(annotations[:,label_ids]==1).sum(dim=1).nonzero(as_tuple=True)[0]
        if strict:
            complement_ids=list(set(range(self._num_labels))-set(label_ids))
            a=set(res_ids.tolist())-set((annotations[:,complement_ids]==1).sum(dim=1).nonzero(as_tuple=True)[0].tolist())
            res_ids=torch.tensor(list(a))
